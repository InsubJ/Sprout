...
export function AuthProvider({ children }: PropsWithChildren): React.JSX.Element {
  ...
  const signInDemo = useCallback(async (identity: DemoIdentity): Promise<void> => {
    await AsyncStorage.setItem(demoKey, JSON.stringify(identity));
    setUser(identity);
  }, []);

  const signInWithGoogleOAuth = useCallback(async (): Promise<boolean> => {
    if (!client)
      throw new Error(
    ...
    if (!code)
      throw new Error(
        `Google's callback did not include an authorization code. Confirm ${redirectTo} is allowed in Supabase Auth.`,
      );
  ...
  const signInWithGoogleIdToken = useCallback(
    async (credential: GoogleIdTokenCredential): Promise<void> => {
      const token = credential.token.trim();
      const nonce = credential.nonce.trim();
      if (!token || !nonce) throw new Error("Google did not return a valid sign-in credential");
      if (!client)
        throw new Error(
          "Google login is not configured. Add the mobile Supabase URL and publishable key first.",
        );
  ...
  const value = useMemo(
    ...
    [loading, signInDemo, signInWithGoogleIdToken, signInWithGoogleOAuth, signOut, user],
  );
