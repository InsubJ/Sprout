...
export function LockScreen() {
  ...
  useEffect(() => {
    if (biometricsEnabled && !started.current) {
      started.current = true;
      void scan();
    }
  }, [biometricsEnabled, scan]);
  const submitPin = async () => {
    if (!/^\d{4}$/.test(pin)) {
      setPinError("PIN code must be exactly 4 digits");
      return;
    }
  ...
  return (
    <View style={[styles.root, { backgroundColor: theme.background }]}>
      <View style={[styles.card, { backgroundColor: theme.surface, borderColor: theme.border }]}>
        ...
        <Text style={[styles.title, { color: theme.text }]}>Sprout Locked</Text>
        <Text style={[styles.copy, { color: theme.muted }]}>
          Please authenticate using your configured lock parameters to access your virtual habits
          canopy.
        </Text>
        {biometric !== "idle" ? (
          <View
            accessibilityLiveRegion="polite"
            style={[styles.biometric, { backgroundColor: theme.elevated }]}
          >
            <Text style={styles.scanIcon}>
              {biometric === "scanning" ? "🧬" : biometric === "success" ? "✔️" : "❌"}
            </Text>
            <Text style={[styles.scanTitle, { color: theme.text }]}>{scanLabel}</Text>
            <Text style={{ color: theme.muted }}>{scanCopy}</Text>
          </View>
        ) : null}
        {pinEnabled ? (
          <View style={styles.pin}>
            ...
            />
            <AppButton label="Unlock Canopy" onPress={() => void submitPin()} />
          </View>
        ) : null}
        {biometricsEnabled ? (
          <AppButton
            label="🧬 Scan Biometrics"
            tone={pinEnabled ? "quiet" : "forest"}
...
const styles = StyleSheet.create({
  ...
  copy: { textAlign: "center", lineHeight: 22, fontFamily: "Outfit_400Regular" },
  pin: { gap: spacing.sm },
  biometric: { padding: spacing.md, borderRadius: 16, alignItems: "center", gap: spacing.xs },
  scanIcon: { fontSize: 38 },
  scanTitle: { fontSize: 18, fontFamily: "Outfit_700Bold" },
});
