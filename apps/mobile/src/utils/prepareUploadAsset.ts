...
export async function prepareUploadAsset(uri: string, operationId: string): Promise<UploadAsset> {
  if (!uri.trim() || !operationId.trim())
    throw new Error("Image URI and operation ID are required");
  const result = await ImageManipulator.manipulateAsync(uri, [{ resize: { width: 1440 } }], {
    compress: 0.72,
