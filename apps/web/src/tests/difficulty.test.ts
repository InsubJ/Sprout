...
describe('Difficulty Utility (difficulty.ts)', () => {
  ...
  describe('Species Mapping', () => {
    it('should return correct species list for each tier', () => {
      expect(getSpeciesForTier('common')).toEqual(['pothos', 'spider_plant', 'jason', 'bonsai', 'maranta_leuconeura', 'lavender']);
      expect(getSpeciesForTier('uncommon')).toEqual(['sunflower', 'alocasia_tiny_dancer', 'remy', 'waratah', 'phalaenopsis_scarlett_jubilee', 'sprig_plant']);
      expect(getSpeciesForTier('rare')).toEqual(['midnight_rose', 'desert_cactus', 'string_of_pearls', 'begonia_maculata', 'poinsettia']);
    ...
    it('should assign species deterministically when index is provided', () => {
      ...
      expect(assignSpecies('common', 2)).toBe('jason');
      expect(assignSpecies('common', 3)).toBe('bonsai');
      expect(assignSpecies('common', 4)).toBe('maranta_leuconeura');
      expect(assignSpecies('common', 5)).toBe('lavender');
      expect(assignSpecies('common', 6)).toBe('pothos'); // wrap around
    });

    it('should assign a valid species from the list when index is not provided', () => {
      const species = assignSpecies('rare');
      expect(['midnight_rose', 'desert_cactus', 'string_of_pearls', 'begonia_maculata', 'poinsettia']).toContain(species);
    });
